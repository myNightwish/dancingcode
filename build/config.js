...
// 遍历组件列表，将组件名字映射到组件库产物lib下对应的文件,使其引用 element-ui/lib 下的对应模块
// （问题todo）：但我没有做组件的按需加载，没有对单个组件打包，而是对整个组件库打包？
Object.keys(Components).forEach(function(key) {
  externals[`mi-element-ui/packages/${key}`] = `mi-selement-ui/lib/${key}`;
});
// 暂不需要
// 遍历 utilsList、mixinsList 和 transitionList，将每个文件设置为对应的外部依赖，使其引用 element-ui/lib 下的对应模块。
// externals['element-ui/src/locale'] = 'element-ui/lib/locale';
// utilsList.forEach(function(file) {
//   file = path.basename(file, '.js');
//   externals[`mi-element-ui/src/utils/${file}`] = `element-ui/lib/utils/${file}`;
// });
